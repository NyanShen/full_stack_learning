...
def build_vector_store():
    # 分割文档
    split_docs = SymTextSplitter().split("data/raw_documents/test.txt")
    SymChromaStoreManager().create_vector_store(split_docs)
        

def build_qa_system(question):
    # 加载本地向量数据库检索器.
    # retriever = SymChromaStoreManager().create_retriever()
    vector_store = SymChromaStoreManager().load_vector_store()
    docs = vector_store.asimilarity_search_with_score(question)
    print("docs:", docs)
    # 检索器检索之后的结果.
